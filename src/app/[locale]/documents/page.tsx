...
export function generateMetadata() {
  return generateMetadataForPath("/documents");
}

export default async function Page() {
  const contentRoot = path.join(process.cwd(), "src", "content");
  const pageContent = loadPageContent(contentRoot, "documents");
  return <HubPageTemplate path="/documents" bodyHtml={pageContent?.bodyHtml} />;
}
