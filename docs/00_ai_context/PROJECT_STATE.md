...
- **Phase 2 in progress (execution track):** **Guest persistence v1** — `src/lib/guest/*`, `src/lib/schemas/guest-blob.ts` (module + schema). **Onboarding `/start` slice v1** — `src/features/onboarding/StartOnboardingFlow.tsx`, `src/lib/onboarding/start-slice.ts` (۴ فیلد §15، merge/prune، resume). **Pure emphasis derivation v1** — `src/lib/onboarding/onboarding-emphasis.ts`: `deriveGuestOnboardingEmphasisV1` → `GuestOnboardingEmphasisV1` (چهار فیلد §15 فقط؛ `has_*` نادیده در v1؛ بدون UI/persist). **Remaining Phase 2:** ادامهٔ onboarding (قدم ۵–۶، نتیجه)، wiring داشبورد/NBA/checklist، trust UI، mapping کامل‌تر per [`ROADMAP_MASTER.md`](../01_strategy/ROADMAP_MASTER.md); see [`CURRENT_PHASE.md`](CURRENT_PHASE.md) / [`CURRENT_FOCUS.md`](CURRENT_FOCUS.md).
