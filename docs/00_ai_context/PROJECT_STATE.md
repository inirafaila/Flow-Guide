...
- **Phase 2 in progress (execution track):** **Guest persistence v1** — `src/lib/guest/*`, `src/lib/schemas/guest-blob.ts` (module + schema). **Onboarding `/start`** — steps **1–5** + **Step 6 Result Summary** in `StartOnboardingFlow.tsx` (consumes **`deriveGuestOnboardingOutcomePreviewV1`**; CTAs **`/dashboard`** + **`/`**; null-preview fallback) + `start-slice.ts`. **Dashboard `/dashboard` NBA v1 (2026-04-06)** — `DashboardNextBestAction.tsx`: client **`readGuestBlob`** + same DTO; loading / no blob / incomplete / valid preview states; **no** full shell/checklist/trust. **Pure emphasis v1** / **signals v2** / **outcome preview v1** unchanged in lib. **Remaining Phase 2:** dashboard shell + checklist filter + trust UI + stay calculator و بقیه per [`ROADMAP_MASTER.md`](../01_strategy/ROADMAP_MASTER.md); see [`CURRENT_PHASE.md`](CURRENT_PHASE.md) / [`CURRENT_FOCUS.md`](CURRENT_FOCUS.md).
