...
- **Phase 2 in progress (execution track):** **Guest persistence v1** — `src/lib/guest/*`, `src/lib/schemas/guest-blob.ts` (module + schema). **Onboarding `/start`** — steps **1–5** + **Step 6 Result Summary** in `StartOnboardingFlow.tsx` (consumes **`deriveGuestOnboardingOutcomePreviewV1`**; CTAs **`/dashboard`** + **`/`**; null-preview fallback) + `start-slice.ts`. **Dashboard `/dashboard` NBA v1 (2026-04-06)** — `DashboardNextBestAction.tsx`: client **`readGuestBlob`** + same DTO; loading / no blob / incomplete / valid preview states; **no** full shell/checklist/trust. **Checklist filtering logic v1 (module-only)** — `src/lib/checklist/checklist-filter.ts` consumed by **`DashboardChecklistBlock`**. **Seed checklist items (8 content fixtures)** under **`src/content/checklist-items/`** (plus Phase 1 sample) — not editorial-final copy. **Checklist row + dashboard checklist block** — **`ChecklistItemRow`** (presentational) composed by **`DashboardChecklistBlock.tsx`** on **`/dashboard`**: server-loaded frontmatter → **`readGuestBlob`** / **`filterChecklistItems`** → category groups → **`ChecklistItemRow`** list (English labels; default **not-started**; no **`UserChecklistStatus`**). **Trust UI components** — **SourceBlock**, **LastVerifiedNote**, **WhatMayVaryNote** for source attribution, verification date, and variance framing. **Guide trust wiring** — **GuidePageTemplate** renders **SourceBlock** + **LastVerifiedNote** + **WhatMayVaryNote** from server-loaded trust data; seed data on **/documents/address-registration**; other guides retain placeholder. **Stay calculator** — **calculateStay** pure 90-in-180 rolling window module + **StayCalculatorBlock** interactive form on **`/documents/stay-calculator`**: add/remove stay entries → **calculateStay** → result (daysUsed / daysRemaining / status / window + watch/urgent warnings). **Phase 2 core exit criteria met** (onboarding, guest blob, dashboard NBA + checklist, guide trust blocks, stay calculator page). **Pure emphasis v1** / **signals v2** / **outcome preview v1** unchanged in lib. **Remaining (incremental, not exit-blocking):** full dashboard shell (header summary, residency card, alerts, feeds), i18n, UserChecklistStatus, more guide trust wiring / source seeding, dashboard residency status card — per [`ROADMAP_MASTER.md`](../01_strategy/ROADMAP_MASTER.md); see [`CURRENT_PHASE.md`](CURRENT_PHASE.md) / [`CURRENT_FOCUS.md`](CURRENT_FOCUS.md).
