...
4. **Observability (Phase 1 wiring)** — **`@sentry/nextjs`** env-gated via root **`sentry.*.config.ts`** + **`src/instrumentation.ts`** (no init without DSNs). Plausible gated on domain + **no default load in `next dev`** unless **`NEXT_PUBLIC_PLAUSIBLE_ENABLE_DEV=true`**. Structured logging baseline and release/source-map automation remain **later** slices.
