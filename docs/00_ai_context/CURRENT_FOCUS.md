...
4. **Observability (Phase 1 wiring)** — **`@sentry/nextjs`** env-gated via root **`sentry.*.config.ts`** + **`src/instrumentation.ts`** (no init without DSNs). Plausible gated on domain + **no default load in `next dev`** unless **`NEXT_PUBLIC_PLAUSIBLE_ENABLE_DEV=true`**. **Minimal structured server logs:** **`src/lib/observability/logger.ts`** (JSON per line); **release/source-map automation** remains a **later** slice.
