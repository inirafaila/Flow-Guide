...
**IA_SPEC.md** §6.1–6.9 page set + airport redirect covered by automated guard (`ia-phase1-routes`); extend types/Zod toward [`DATA_CONTENT_MODEL_SPEC.md`](../02_product/DATA_CONTENT_MODEL_SPEC.md) as needed; **Epic 1.4** / **Epic 2.4–2.5** partially addressed (**2026-04-04** minimal tokens + **`Button`/`Card`/`SectionHeader`** on shell/template placeholders only)—**no** Phase 2 **behavior**; local **lint / test / build** green; PR CI runs the same **lint / test / build**; Sentry/Plausible **env-gated wiring** + **minimal structured logging** shipped (2026-04-03 / 2026-04-04)—**later:** source maps / `withSentryConfig`, funnel events; **no** NBA/checklist/guest logic beyond constants/stubs. Optional `?lang=` (deferred per `ENGINEERING_ARCHITECTURE.md` §7).
